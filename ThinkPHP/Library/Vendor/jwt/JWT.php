...
class JWT
{
    ...
    protected function error($info){
        return array('iRet'=>0, 'info'=>$info);
    }

    protected function success($data, $info){
        return array('iRet'=>1, 'info'=>$info, 'data'=>$data);
    }
    ...
    {
        if (empty($key)) {
            // throw_exception('Key may not be empty');
            return array('iRet'=>0, 'info'=>'Key may not be empty');
        }
        $tks = explode('.', $jwt);
        if (count($tks) != 3) {
    ...
    public static function sign($msg, $key, $alg = 'HS256')
    {
        ...
        switch($function) {
            ...
            case 'openssl':
                ...
                if (!$success) {
                    throw_exception("OpenSSL unable to sign data");
                } else {
                    return $signature;
                }
        }
    ...
    private static function verify($msg, $signature, $key, $alg)
    {
        ...
        switch($function) {
            case 'openssl':
                ...
                if (!$success) {
                    throw_exception("OpenSSL unable to verify data: " . openssl_error_string());
                } else {
                    return $signature;
                }
            case 'hash_hmac':
    ...
    public static function jsonDecode($input)
    {
        ...
        if (function_exists('json_last_error') && $errno = json_last_error()) {
            ...
            throw_exception('Null result with non-null input');
        }
        return $obj;
    }
