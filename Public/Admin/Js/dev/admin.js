...
$('#J_B_main_block').on('click', 'a', function(e) {
	...
	if (data_id == 'content') {
		$('.menubar dl').addClass('tree_scroll');
		$('.menubar dl').css('height', $('.content').height() - 21);
		show_left_tree();
	} else {
		$('.menubar dl').removeClass('tree_scroll');
	...
	function show_left_tree() {
		$.ajax({
			...
			success: function(data) {
				...
				$.fn.zTree.init($("#B_menubar"), setting, data);
			}
		});
	}
...
function iframeJudge(options) {
	...
	if (li.length > 0) {
		...
		$(iframe[0].contentWindow.document).ready(function() {
			...
			var li = $('<li tabindex="0"><span><a>' + elem.text() + '</a><a class="del" title="关闭此页">关闭</a></span></li>').attr('data-id', id).addClass('current');
