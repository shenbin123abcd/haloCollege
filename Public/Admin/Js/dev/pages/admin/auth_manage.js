        ...
        beforeSubmit : function(){
          ...
          var org_text = auth_sub.text();
       		auth_sub.text(org_text.replace(/(中...)$/, '')).parent().find('span').remove();
					...
					if (data.status == 1) {
						$('<span class="tips_success">' + data.info + '</span>' ).appendTo(auth_sub.parent()).fadeIn('slow').delay( 1000 ).fadeOut(function() {
							reloadPage(window.parent);
						});
					}else if(data.status == 0){
						$('<span class="tips_error">' + data.info + '</span>' ).appendTo(auth_sub.parent()).fadeIn( 'fast' );
