...
gulp.task('sass', function () {
    return gulp.src([appConfig.themeSrc + '/css/*.scss']).pipe(sourcemaps.init()).pipe(plugins.sass({ outputStyle: 'compact' }).on('error', plugins.sass.logError)).pipe(sourcemaps.write()).pipe(gulp.dest(appConfig.themeSrc + '/css'));
});

gulp.task('copy:css', ['sass'], function () {
    return gulp.src([appConfig.themeSrc + '/css/**/*.{css,map}']).pipe(gulp.dest(appConfig.themeDist + '/css'));
});

gulp.task('copy:js', function () {
    return gulp.src([appConfig.themeSrc + '/js/*.js']).pipe(plugins.cached('myjs')).pipe(plugins.cdnizer({
...
gulp.task('images', function () {
    ...
    .pipe(plugins.rev()).pipe(gulp.dest(appConfig.themeDist + '/images')).pipe(plugins.rev.manifest()).pipe(gulp.dest('tmp/images'));
});
...
gulp.task('build', ['sass', 'images', 'webpack'], function () {
    ...
    return gulp.src('app/index.html').pipe(plugins.useref()).pipe(jsFilter).pipe(plugins.revReplace({ manifest: manifestJs })).pipe(plugins.cdnizer({
        defaultCDNBase: '/Public/Ke',
        //defaultCDNBase: "../",
        allowRev: true,
        allowMin: true,
        ...
        fallback: false,
        files: ['/images/**/*']
    })).pipe(plugins.rev()).pipe(plugins.babel({
        presets: ['es2015']
    })).pipe(plugins.uglify()).pipe(gulp.dest('' + appConfig.themeDist)).pipe(jsFilter.restore).pipe(cssFilter).pipe(plugins.revReplace({ manifest: manifestCss })).pipe(plugins.cdnizer({
        defaultCDNBase: '/Public/Ke',
        ...
        // ],
        // fallback: false,
        files: ['images/**/*.{jpg,png,mp3,mp4}']
    })).pipe(plugins.autoprefixer({
        browsers: ['> 0%'],
        cascade: false
    })).pipe(plugins.csso()).pipe(plugins.rev())
    // .pipe(gulp.dest('./tmp'))
    .pipe(gulp.dest('' + appConfig.themeDist)).pipe(cssFilter.restore).pipe(plugins.revReplace({
        replaceInExtensions: ['.js', '.css', '.html', '.ejs']
    })).pipe(htmlFilter).pipe(plugins.revReplace({ manifest: manifestHtml })).pipe(plugins.cdnizer({
...
gulp.task('copy:view', ['copy:css'], function () {
    var htmlFilter = plugins.filter('**/*.html', { restore: true });
    return gulp.src([appConfig.themeSrc + 'app/**/*.html']).pipe(htmlFilter).pipe(plugins.cdnizer({
        defaultCDNBase: '/',
        //defaultCDNBase: "../",
        allowRev: true,
        allowMin: true,
...
gulp.task('clean', require('del').bind(null, [appConfig.themeDist + '/*', appConfig.themeViewDist + '/*'], { force: true }));
...
gulp.task("webpack:ewteert", function (callback) {
    ...
    return gulp.src('app/entry.js').pipe(webpackStream(webpackConfig)).pipe(gulp.dest('tmp'));
});
