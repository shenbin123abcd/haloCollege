...
class CommonFinalMatch extends React.Component{
    ...
    renderGoldenBearVideos(){
        const { dispatch,location,match_final,id,type,level} = this.props;
        if(match_final){
            return(
                <div className="common-article-wrapper">
                    <div className="article-top">
                        <div className="article-title">决赛</div>
                        <div className="article-arrow">
                            <svg className="haloIcon" aria-hidden="true">
                                <use xlinkHref="#haloIcon-arrow2"></use>
                            </svg>
                        </div>
                        <Link to={`/share/goldBearsVideosList/${id}/${type}/${level}`} className="article-watch">查看所有{match_final.total}个视频</Link>
                    </div>
                    {
                        match_final.list.map((n,i)=>{
                            return(
                                <a href={`http://college-api.halobear.com/video/detail?id=${n.id}`} className="article-detail"  key={n.id}>
                                    ...
                                    <div className={`myLine ${(match_final.list.length==(i+1))?'lineLast':''}`} ></div>
                                </a>
                            )
                        })
                    }
    ...
    render(){
        const { dispatch,location,matchFinal} = this.props;
        return(
            <span>
...
export default ReactRedux.connect(mapStateToProps)(CommonFinalMatch)